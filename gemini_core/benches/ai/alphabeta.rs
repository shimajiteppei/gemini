...
/// `alphabeta::Agent::select_move` を計測する（depth=3）。
fn bench_select_move_depth_3(criterion: &mut Criterion) {
    let samples = position_samples();
    let mut group = criterion.benchmark_group("ai/alphabeta/select_move_depth_3");
...
fn main() {
    ...
    bench_select_move_depth_3(&mut criterion);
